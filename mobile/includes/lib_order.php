...
function cart_goods($type = CART_GENERAL_GOODS)
{
    ...
    foreach ($arr as $key => $value)
    {
        ...
        $sql="select goods_img,goods_thumb from ecs_goods where goods_id=".$value['goods_id'];
        $res=$GLOBALS['db']->getRow($sql);
        $arr[$key]['goods_img']=$res['goods_img'];
        $arr[$key]['goods_thumb']=$res['goods_thumb'];
    }
...
function get_cart_goods()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['market_total'] = price_format($row['market_price']* $row['goods_number'],false);
