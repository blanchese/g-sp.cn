...
/* 获得页面的缓存ID */
    ...
    $sql="select * from ecs_article_cat where cat_id=".$cat_id;
    $catinfo=$db->getRow($sql);
    $smarty->assign('catinfo',$catinfo);
    $temp='list_'.$catinfo['type'].".html";
    
    if($catinfo['parent_id']>0){
	    $sql="select cat_name,cat_id from ecs_article_cat where parent_id=".$catinfo['parent_id']." order by sort_order asc,cat_id asc";
	    $art_cat_list=$db->getAll($sql);
	    $smarty->assign('art_cat_list',$art_cat_list);
    }
    
    if($catinfo['type']=='main'){
    	$sql="select cat_id from ecs_article_cat where parent_id=".$cat_id." order by sort_order asc,cat_id asc";
    ...
    if($catinfo['type']=='honor'){
    	...
    	$artciles_list=array_chunk($article_list, 2);
    	$smarty->assign('honnor_list',    $artciles_list);
    }
