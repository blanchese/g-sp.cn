...
if (!$smarty->is_cached('product_view.html', $cache_id))
{
...
/*        $smarty->assign('properties',          $properties['pro']);                              // 商品属性
		...
		$picurl_list=get_goods_gallery($goods_id);
		foreach ($picurl_list as $k=>$v){
			$picurl_list[$k]['img_url']='../'.$v['img_url'];
		}
 		$smarty->assign('pictures', $picurl_list);                    // 商品相册
...
$sql="select goods_id,goods_name,goods_img,shop_price,market_price from ecs_goods where is_hot=1 and cat_id=".$goods['cat_id']." order by click_count desc limit 10";
$hot_list=$db->getAll($sql);
...
$smarty->display('product_view.html',      $cache_id);
