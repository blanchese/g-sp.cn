...
function get_ad_list($position_id,$num=1){
	$sql="select * from ecs_ad where position_id=".$position_id." and start_time <".gmtime()." and end_time >".gmtime()." and enabled=1 order by start_time desc limit ".$num;
	if($num>1){
...
function get_volume_price_list($goods_id, $price_type = '1')
{
    ...
    foreach ($res as $k => $v)
    {
        ...
        if($i>$k && $i<count($res)){
        	...
        	$volume_price[$temp_index]['number_str']   =  $v['volume_number'].' 以上';
        }
