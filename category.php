...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update','salesnum'))) ? trim($_REQUEST['sort'])  : 'salesnum';
...
//-- PROCESSOR
/*------------------------------------------------------ */
$smarty->assign('sort_flag',$sort);
$smarty->assign('order_flag',$order);
/* 页面的缓存ID */
  ...
  /*      ecs_header("Location: ./\n");
    ...
    $count = get_cagtegory_goods_count($children, $brand, $price_min, $price_max,$keywords, $ext);
    ...
    $next_page=$page+1 > $max_page ? $max_page :$page+1;
    ...
    if(!empty($cat) && $cat['parent_id']==0){
    	$sql="select cat_name,cat_id from ecs_category where parent_id=".$cat['cat_id']." and is_show=1 order by sort_order asc,cat_id asc ";
		$product_cat=$GLOBALS['db']->getAll($sql);
		$smarty->assign('product_cat',$product_cat);
    }elseif(!empty($cat) && $cat['parent_id']>0){
    	$sql="select cat_name,cat_id from ecs_category where parent_id=".$cat['parent_id']." and is_show=1 order by sort_order asc,cat_id asc ";
		$product_cat=$GLOBALS['db']->getAll($sql);
		$smarty->assign('product_cat',$product_cat);
    }else{
    	if(!empty($product_cat_list)){
    		$category_ad=get_ad_list(5,count($product_cat_list));
    		foreach ($product_cat_list as $k=>$v){
		    	$sql="select cat_name,cat_id from ecs_category where parent_id=".$v['cat_id']." and is_show=1 order by sort_order asc,cat_id asc ";
				$product_cat_list[$k]['cat_list']=$GLOBALS['db']->getAll($sql);
    			$sql="select g.goods_name,g.goods_img,g.shop_price,g.market_price,g.goods_id from ecs_goods as g  where g.is_on_sale=1 and g.is_delete=0 and  is_best=1 and ".get_children($v['cat_id'])." order by click_count desc limit 6";
    			$product_cat_list[$k]['hot_list']=$GLOBALS['db']->getAll($sql);
    			$i=$k+1;
    			$product_cat_list[$k]['ad_code']=$category_ad[$i]['ad_code'];
    			$product_cat_list[$k]['ad_link']=$category_ad[$i]['ad_link'];
    			$product_cat_list[$k]['ad_name']=$category_ad[$i]['ad_name'];
    		}
    		$smarty->assign('product_cat_list',$product_cat_list);
    	}
    	$smarty->assign('banner_ad',get_ad_list(4,1));
    }
    if($cat_id>0){
	    $sql="select goods_name,goods_id,goods_img,shop_price,market_price from ecs_goods  as g where is_delete=0 and is_on_sale=1 and is_new=1 and ".get_children($cat_id)." order by click_count desc limit 12";
	    $new_pro_list=$db->getAll($sql);
	    $smarty->assign('new_pro_list',$new_pro_list);
    }
	
    if(!empty($keywords)){
		$temp="product_search.html";
	}
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name,cat_id, keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order,$keywords='')
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['market_price']     = number_format($row['market_price'], 2, '.', '');
 ...
 * @return  integer
 */
function get_cagtegory_goods_count($children, $brand = 0, $min = 0, $max = 0,$keywords='', $ext='')
{
