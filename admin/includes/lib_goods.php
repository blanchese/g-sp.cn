...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price,v.attr_picurl ".
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        if($val['attr_type'] == 1 || $val['attr_type'] == 2){    
        	$html .='&nbsp;&nbsp;上传图片：<input type="file" name="attr_picurl_list[]" /> &nbsp;&nbsp;'. $val['attr_picurl'];   
        } 
