...
*/
    ...
    else
    {
		...
		//edit by mivec  //choice the thumbnail from gallery
		//print_r($_POST);
		$_imgId = $_POST['is_thumbnail'];
		if (!empty($_imgId)) {
			$_imgSql = "SELECT * FROM " . $ecs->table('goods_gallery') . " WHERE img_id=$_imgId";
			$_imgRow = $db->getRow($_imgSql);
			//print_r($_imgRow);exit;
			//update good's images;
			$sql .= "goods_thumb = '".$_imgRow['thumb_url']."',goods_img = '".$_imgRow['img_url']."', original_img = '".$_imgRow['img_original']."', ";
		}
    ...
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
        ...
        if(isset($_POST['attr_id_list']))
        {
        	foreach ($_POST['attr_id_list'] AS $key => $attr_id)
        	{
        		...
        		//$attr_picurl=  basename($image->upload_image($_POST['attr_picurl_list'][$key],'brandlogo'));
        		$image_files=$_FILES['attr_picurl_list'];
        		$upload = array(
	            ...
	            $attr_picurl_old = $GLOBALS['image']->upload_image($upload);
	            $attr_picurl = reformat_image_name('gallery', $goods_id, $attr_picurl_old, 'goods');
                ...
                if (!empty($attr_value))
                {
                    if (isset($goods_attr_list[$attr_id][$attr_value]))
                    {
                        ...
                        $goods_attr_list[$attr_id][$attr_value]['attr_picurl'] = $attr_picurl;
                    }
                    else
                    {
                        // 如果原来没有，标记为新增
        ...
        foreach ($goods_attr_list as $attr_id => $attr_value_list)
        {
            foreach ($attr_value_list as $attr_value => $info)
            {
                if ($info['sign'] == 'insert')
                {
                	if(!empty($info['attr_picurl'])){
                    	$sql = "INSERT INTO " .$ecs->table('goods_attr'). " (attr_id, goods_id, attr_value, attr_price,attr_picurl)".
                            	"VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]','$info[attr_picurl]')";
                	}else{
                ...
                {
                	if(!empty($info['attr_picurl'])){
                		$sql = "UPDATE " .$ecs->table('goods_attr'). " SET attr_price = '$info[attr_price]',attr_picurl='$info[attr_picurl]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                	}else{
