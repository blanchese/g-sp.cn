...
/* 页面的缓存ID */
  ...
  /*      ecs_header("Location: ./\n");
    ...
    foreach ($goodslist as $k=>$v){
    	$goodslist[$k]['goods_img']='../'.$v['goods_img'];
    ...
    if(!empty($cat) && $cat['parent_id']==0){
    	...
    	if(!empty($product_cat_list)){
    		...
    		foreach ($product_cat_list as $k=>$v){
    			...
    			$sql="select g.goods_name,g.goods_img,g.shop_price,g.market_price,g.goods_id from ecs_goods as g  where g.is_best=1 and ".get_children($v['cat_id'])." order by click_count desc limit 6";
