...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','ajax_email','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 
'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer','ajax_city','ajax_check_code','change_pwd','reset_pwd','ajax_mobile_code');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list','change_password',
...
if($action=='ajax_city'){
	...
	echo $str;
	exit;
}elseif($action=='ajax_mobile_code'){
	$mobile    = !empty($_POST['mobile'])    ? trim($_POST['mobile'])     : '';
	$code=rand(11111, 99999);
	$res=1;
	if($res){
		$_SESSION['mobile_code']=md5($code);
		$_SESSION['check_mobile']=$mobile;
		echo 1;
	}else{
		echo '短信发送失败！';
	}	
...
elseif ($action == 'ajax_email')
{
	...
	if(is_email($email)){
		$subject='【冠思品商城】邮箱验证码';
		$content='尊敬的用户,您好！<br><br>&nbsp;&nbsp;&nbsp;&nbsp;验证码为：'.$code.'。验证码有效为30分钟。 如非本人操作，请忽略此邮件。<br><br>【冠思品商城】';
	  	if(send_mail($email, $email, $subject, $content,1)){
	...
	if($type>0){
		...
		$code_check=$_SESSION['email_code'];
		if(is_email($email)){
			if($check_email==$email){
				$sql="select user_id from ecs_users where email='".$email."'";
				$uid=intval($db->getOne($sql));
				if($uid >0){
					if($code_check !=md5($code)){
						echo '激活码错误！';
					}else{
						unset($_SESSION['email_code']);
						unset($_SESSION['check_email']);
						$_SESSION['change_user_id']=$uid;
						echo 1;
					}
				}else{
					echo '邮箱不存在！';
		...
		$check_mobile=$_SESSION['check_mobile'];
		$mobile_check=$_SESSION['mobile_code'];
		if(!empty($mobile)){
			$sql="select user_id from ecs_users where mobile_phone='".$mobile."'";
			$uid=intval($db->getOne($sql));
			if($uid >0){
				if($mobile_check !=md5($code)){
					echo '激活码错误！';
				}else{
	...
	$change_user_id=intval($_SESSION['change_user_id']);
	if($change_user_id <1){
		ecs_header('Location:user.html?act=get_password');
		exit;
	}
    $position = assign_ur_here(0, $_LANG['user_center']);
    $smarty->assign('page_title', $position['title']); // 页面标题
    $smarty->assign('ur_here',    $position['ur_here']);
	$smarty->display('user_change_pwd.html');
}elseif($action=='reset_pwd'){
	$password=$_REQUEST['password'];
	$confirm_password=$_REQUEST['confirm_password'];
	$uid=intval($_SESSION['change_user_id']);
	if($uid>0){
		if($password != $confirm_password){
			show_message('前后输入的密码不一样！','','','error');
		}else{
			$sql="update ecs_users set password='".md5($password)."' where user_id=".$uid;
			if($db->query($sql)){
				unset($_SESSION['change_user_id']);
				show_message('恭喜您成功找回密码，请重新登录！','会员中心','user.html','info');
			}else{
				show_message('对不起找回密码失败，请重新找回！','会员中心','user.html?act=get_password','error');
			}
		}
	}else{
		ecs_header('Location:user.html?act=get_password');
...
if ($action=='change_password'){
	...
	$smarty->display('user_password.html');
}
/* 显示会员注册界面 */
if ($action == 'register')
{
...
elseif ($action == 'act_register')
{
    ...
    if ($_CFG['shop_reg_closed'])
    {
        ...
        show_message('很抱歉，由于特殊原因，本商城暂时关闭会员注册','','','error');
    }
    else
    {
        include_once(ROOT_PATH . 'includes/lib_passport.php');
        ...
        {
            //show_message($_LANG['passport_js']['agreement'],'','','error');
        }
        if (strlen($username) < 3)
        {
            show_message($_LANG['passport_js']['username_shorter'],'','','error');
        }
        ...
        if(!empty($other['mobile_phone'])){
	        $sql = "SELECT user_id FROM ecs_users  WHERE mobile_phone = '".$other['mobile_phone']."'";
	        if ($db->getOne($sql, true) > 0)
	        {
	            show_message('手机号码已被注册！', '', '', 'error');
	        }
        }
        
        $register_type=trim($_REQUEST['register_type']);
        $code=md5(trim($_REQUEST['code']));
        $code_check=$_SESSION['email_code'];
        if($register_type=='email'){
			$check_email=$_SESSION['check_email'];
			if($check_email==$email && is_email($email)){
				if($code!=$code_check){
					show_message('激活码不正确！','','','error');
				}
			}else{
				show_message('前后邮箱不一致！','','','error');
			}
        }else{
/*        	$check_mobile=$_SESSION['check_mobile'];
        	if($check_mobile==$other['mobile_phone'] && !empty($other['mobile_phone'])){
				if($code!=$code_check){
					show_message('激活码不正确！','','','error');
				}
			}else{
				show_message('前后手机号码不一致！','','','error');
			}*/
        }
        ...
        if (register($username, $password, $email, $other) !== false)
        {
            ...
            unset($_SESSION['mobile_code']);
            unset($_SESSION['email_code']);
            $smarty->assign('content',sprintf($_LANG['register_success'], $username . $ucdata));
...
elseif ($action == 'login')
{
	...
	$back_act=str_ireplace('logout.html', 'user.html', $back_act);
...
elseif ($action == 'act_edit_profile')
{
    ...
    {
        show_message($_LANG['edit_profile_success'], $_LANG['profile_lnk'], 'user.html', 'info');
    }
    else
    {
...
elseif ($action == 'get_password')
{
    ...
    {
        //显示用户名和email表单
        $smarty->display('user_get_password.html');
    }
}
...
elseif ($action == 'order_list')
{
    ...
    if($status==1){
    	...
    	$where=' and pay_status !=2 and order_status !=2';
    }elseif($status==3){
    ...
    $pager  = get_pager('user.html', array('act' => $action,'status'=>$status), $record_count, $page,3);
    
    $tt_count=ceil($record_count/3);
    $next_page=($page+1) > $tt_count ? $tt_count : ($page+1);
    ...
    foreach ($orders as $k=>$v){
		$sql="select o.goods_id,o.goods_name,o.goods_number,o.goods_price,o.market_price,g.goods_img from ecs_order_goods as o left join ecs_goods as g on g.goods_id=o.goods_id where order_id=".$v['order_id']; 
		$res=$db->getAll($sql); 
		foreach ($res as $i=>$vo){
			$res[$i]['goods_img']='../'.$vo['goods_img'];
		}  	
		$v['list']=$res;
		$v['handler']=str_ireplace('取消订单', '取消', $v['handler']);
    	$order_list[$k]=$v;
    }
	 //var_dump($orders);
	$total_count=intval($db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE user_id = '$user_id'"));
	$cancel_count=intval($db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE user_id = '$user_id' and order_status=2 "));
	$pay_count=intval($db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE user_id = '$user_id' and pay_status=2 "));
	$npay_count=intval($db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE user_id = '$user_id' and pay_status !=2 and order_status !=2"));
	$smarty->assign('total_count',  $total_count);
	$smarty->assign('cancel_count',  $cancel_count);
	$smarty->assign('pay_count',  $pay_count);
	$smarty->assign('npay_count',  $npay_count);
    $smarty->assign('merge',  $merge);
    $smarty->assign('pager',  $pager);
    $smarty->assign('page',  $page);
    $smarty->assign('status',  $status);
    $smarty->assign('orders', $order_list);
    $smarty->display('user_order.html');
}
...
elseif ($action == 'order_detail')
{
    ...
    foreach ($goods_list AS $key => $value)
    {
        ...
        $goods_list[$key]['goods_img']    ='../'.$db->getOne('select goods_img from ecs_goods where goods_id='.$value['goods_id']);
    }
    ...
    $smarty->display('user_order_view.html');
}
