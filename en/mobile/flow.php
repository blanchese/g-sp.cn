 ...
 * ============================================================================
 * $Author: douqinghua $
 * $Id: cart.html 17218 2011-01-24 04:10:41Z douqinghua $
 */
...
// 动态ajax改变购物车中的数据
if($_REQUEST['action']=='ajax_update_cart'){  
	...
	if(empty($rec_id) || empty($goods_number)){
		$result['error'] = 1;
		$result['message'] = '数据丢失，请联系网站客服人员';
        die($json->encode($result));
	}
	/* 检查：库存 */
	if ($GLOBALS['_CFG']['use_storage'] == 1){
		$sql="select product_id,goods_id,goods_attr_id from ecs_cart where rec_id=$rec_id";
		$info=$GLOBALS['db']->getRow($sql);
		$sql="select goods_number,cat_id from ecs_goods where goods_id =".$info[goods_id];
		$gd=$GLOBALS['db']->getRow($sql);
		//检查：商品购买数量是否大于总库存
		if ($goods_number > $gd['goods_number']){
			$result['error'] = 1;
			$result['message'] = '商品库存不足！';
	        die($json->encode($result));
		}
	}
	$sql="select goods_id from ecs_cart where rec_id = $rec_id";
	$goods_id=$db->getOne($sql);
	$goods_price = get_final_price($goods_id, $goods_number, true, $spec);
	
	$sql = "update ecs_cart set goods_number = $goods_number,goods_price= $goods_price where rec_id = $rec_id limit 1";
 
	if($db->query($sql)){
		$res=get_cart_goods();
		$result['content']=$res['total'];
		$result['price']=$goods_price;
		die($json->encode($result));
	}else{
		$result['error'] = 1;
		$result['message'] = '购物车更新失败，请手动刷新！';
	    die($json->encode($result));
	}
    ...
    else
    {
        ...
        if (!empty($_POST['act']) && $_POST['act'] == 'signin')
        {
            ...
            if ($user->login($_POST['username'], $_POST['password'],isset($_POST['remember'])))
            {
                ...
                if ($db->getOne($sql) > 0)
                {
                    ecs_header("Location: cart.html?step=checkout\n");
                }
                else
                {
            ...
            {
                $_SESSION['login_fail']++;
                show_message($_LANG['signin_failed'], '', 'cart.html?step=login');
            }
        }
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            {
                /* 用户注册成功 */
                ecs_header("Location: cart.html?step=consignee\n");
                exit;
            }
            else
            {
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
       ...
       // $smarty->assign('country_list',       get_regions());
        $smarty->assign('sheng_list',       get_regions(1,1));
...
/*        foreach ($consignee_list as $region_id => $consignee)
        ...
        $province_list = get_regions(1, $consignee_list['country']);
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if (empty($_SESSION['direct_shopping']) && $_SESSION['user_id'] == 0)
    {
    	...
    	ecs_header("Location: cart.html?step=login\n");
    	exit;
    }
    ...
    foreach ($cart_goods as $k=>$v){
    	$cart_goods[$k]['goods_thumb']='../'.$v['goods_thumb'];
    }
...
elseif ($_REQUEST['step'] == 'update_cart')
{
    ...
    show_message($_LANG['update_cart_notice'], $_LANG['back_to_cart'], 'cart.html');
    exit;
}
...
{
    $rec_id = $_GET['id'];
    $arr=array_filter(explode(',', $rec_id));
    if(!empty($arr)){
		foreach ($arr as $v){
			flow_drop_cart_goods(intval($v));
		}    	
    }
    ecs_header("Location: cart.html\n");
    exit;
...
else
{
    ...
    foreach ($cart_goods['goods_list'] as $k=>$v){
    	$cart_goods['goods_list'][$k]['goods_thumb']='../'.$v['goods_thumb'];
    }
