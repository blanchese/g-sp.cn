...
//require('../includes/lib_article.php');
...
/* 载入语言文件 */
require('../languages/' . $_CFG['lang'] . '/common.php');
...
$sql="select cat_name,cat_id from ecs_category where parent_id=0 and is_show=1 order by sort_order asc,cat_id asc limit 8";
...
$sql="select cat_name,cat_id from ecs_article_cat where parent_id=0 and show_in_nav=1 order by sort_order asc limit 4";
$footer_nav=$GLOBALS['db']->getAll($sql);
foreach ($footer_nav as $k=>$v){
	$sql="select cat_name,cat_id from ecs_article_cat where parent_id =".$v['cat_id']." order by sort_order asc";
