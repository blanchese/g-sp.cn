...
if($_REQUEST['action']=='ajax_update_cart'){  
	...
	if ($GLOBALS['_CFG']['use_storage'] == 1){
		$sql="select product_id,goods_id,goods_attr_id from en_cart where rec_id=$rec_id";
		$info=$GLOBALS['db']->getRow($sql);
		$sql="select goods_number,cat_id from en_goods where goods_id =".$info[goods_id];
	...
	$sql="select goods_id from en_cart where rec_id = $rec_id";
	$goods_id=$db->getOne($sql);
	$goods_price = get_final_price($goods_id, $goods_number, true, $spec);
	
	$sql = "update en_cart set goods_number = $goods_number,goods_price= $goods_price where rec_id = $rec_id limit 1";
