...
function load_config()
{
    ...
    if ($data === false)
    {
        ...
        if (!isset($GLOBALS['_CFG']['en_version']))
        {
            /* 如果没有版本号则默认为2.0.5 */
            $GLOBALS['_CFG']['en_version'] = 'v2.0.5';
        }
...
function get_ad_list($position_id,$num=1){
	$sql="select * from en_ad where position_id=".$position_id." and start_time <".gmtime()." and end_time >".gmtime()." and enabled=1 order by start_time desc limit ".$num;
	if($num>1){
		$ad_list1=$GLOBALS['db']->getAll($sql);
		foreach ($ad_list1 as $k=>$v){
			$i=$k+1;
			$ad_list[$i]=$v;
		}
	}else{
		$ad_list=$GLOBALS['db']->getRow($sql);
	}
	return $ad_list;
}
/**
...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    ...
    switch ($app)
    {
        ...
        case 'goods':
            ...
            {
                $uri = $rewrite ? 'pro_view-' . $gid : 'goods.php?id=' . $gid;
            }
 ...
 * @return  mix
 */
function article_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0,$flag=0)
{
    static $res = NULL;
    ...
    if ($re_type == true)
    {
        ...
        foreach ($options AS $var)
        {
            ...
            $select .= ($selected == $var['cat_id']) ? "selected='ture'" : '';
            if($flag==1){
	            if(in_array($var['type'], array('page','main','contact','sitemap'))){
...
function get_volume_price_list($goods_id, $price_type = '1')
{
    ...
    foreach ($res as $k => $v)
    {
        ...
        $i=$k+1;
        if($i>$k && $i<count($res)){
        	$volume_price[$temp_index]['number_str']   = $v['volume_number'].'-'.($res[$i]['volume_number']-1);
        }else{
        	$volume_price[$temp_index]['number_str']   =  $v['volume_number'].' More than';
        }
