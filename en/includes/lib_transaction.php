...
function get_profile($user_id)
{
    ...
    $sql  = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
             " msn, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer,email ".
...
function get_user_orders($user_id, $num = 10, $start = 0,$where='')
{
    ...
    $sql = "SELECT order_id, order_sn, order_status, pay_name,shipping_status, pay_status, add_time, " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $row['handler'] = "<a href=\"user.html?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>";
        }
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            {
                @$row['handler'] = "<a href=\"user.html?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\">".$GLOBALS['_LANG']['received']."</a>";
            }
            ...
            else
            {
                ...
                {
                    @$row['handler'] = "<a href=\"user.html?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['pay_money']. '</a>';
                }
                else
                {
                    @$row['handler'] = "<a href=\"user.html?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['view_order']. '</a>';
                }
        ...
        $arr[] = array('order_id'       => $row['order_id'],
        			   ...
        			   'hour_time'      => local_date('H:i:s', $row['add_time']),
...
function get_user_bouns_list($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $row['status'] = '<a href="user.html?act=order_detail&order_id=' .$row['order_id']. '" >' .$GLOBALS['_LANG']['had_use']. '</a>';
        }
