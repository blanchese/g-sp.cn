...
/* 页面的缓存ID */
  ...
  /*      ecs_header("Location: ./\n");
    ...
    if(!empty($cat) && $cat['parent_id']==0){
    	$sql="select cat_name,cat_id from en_category where parent_id=".$cat['cat_id']." and is_show=1 order by sort_order asc,cat_id asc ";
		$product_cat=$GLOBALS['db']->getAll($sql);
		$smarty->assign('product_cat',$product_cat);
    }elseif(!empty($cat) && $cat['parent_id']>0){
    	$sql="select cat_name,cat_id from en_category where parent_id=".$cat['parent_id']." and is_show=1 order by sort_order asc,cat_id asc ";
		$product_cat=$GLOBALS['db']->getAll($sql);
		$smarty->assign('product_cat',$product_cat);
    }else{
    	if(!empty($product_cat_list)){
    		...
    		foreach ($product_cat_list as $k=>$v){
		    	$sql="select cat_name,cat_id from en_category where parent_id=".$v['cat_id']." and is_show=1 order by sort_order asc,cat_id asc ";
				$product_cat_list[$k]['cat_list']=$GLOBALS['db']->getAll($sql);
    			$sql="select g.goods_name,g.goods_img,g.shop_price,g.market_price,g.goods_id from en_goods as g  where g.is_on_sale=1 and g.is_delete=0 and  is_best=1 and ".get_children($v['cat_id'])." order by click_count desc limit 6";
    ...
    if($cat_id>0){
	    $sql="select goods_name,goods_id,goods_img,shop_price,market_price from en_goods  as g where is_delete=0 and is_on_sale=1 and is_new=1 and ".get_children($cat_id)." order by click_count desc limit 12";
