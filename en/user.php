...
if($action=='ajax_city'){
	...
	$sql="select * from en_region where parent_id=".$id;
	$list=$db->getAll($sql);
	$str='';
	if($ty>0){
...
elseif ($action == 'ajax_email')
{
	...
	if($type>0){
		...
		if(is_email($email)){
			if($check_email==$email){
				$sql="select user_id from en_users where email='".$email."'";
		...
		if(!empty($mobile)){
			$sql="select user_id from en_users where mobile_phone='".$mobile."'";
			$uid=intval($db->getOne($sql));
			if($uid >0){
	...
	if($uid>0){
		if($password != $confirm_password){
			...
			$sql="update en_users set password='".md5($password)."' where user_id=".$uid;
			if($db->query($sql)){
...
if ($action == 'default')
{
    ...
    $sql="select * from en_users where user_id=".$user_id;
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        if(!empty($other['mobile_phone'])){
	        $sql = "SELECT user_id FROM en_users  WHERE mobile_phone = '".$other['mobile_phone']."'";
...
elseif ($action == 'order_list')
{
    ...
    $pager  = get_pager('user.html', array('act' => $action,'status'=>$status), $record_count, $page);

    $orders = get_user_orders($user_id, $pager['size'], $pager['start'],$where);
    $merge  = get_user_merge($user_id);
    
    foreach ($orders as $k=>$v){
		$sql="select o.goods_id,o.goods_name,o.goods_number,o.goods_price,g.goods_img from en_order_goods as o left join en_goods as g on g.goods_id=o.goods_id where order_id=".$v['order_id']; 
		$v['list']=$db->getAll($sql);   	
    	$order_list[$k]=$v;
    }
	 //var_dump($orders);
...
elseif ($action == 'order_detail')
{
    ...
    foreach ($goods_list AS $key => $value)
    {
        ...
        $goods_list[$key]['goods_img']    =$db->getOne('select goods_img from en_goods where goods_id='.$value['goods_id']);
    }
